...
import com.ishland.c2me.opts.accel.vulkan.common.gen.VkServerBatchedBiomeNoiseContext;
...
public class BatchingBiomeNoiseStatus extends NewChunkStatus {
    ...
    public Completable upgradeToThis(ChunkLoadingContext context, Cancellable cancellable) {
        ...
        if (!VkServerBatchedBiomeNoiseContext.isAligned(pos.x(), pos.z()) || startingChunk.getStatus().isAtLeast(ChunkStatus.NOISE)) {
        ...
        VkServerWorldContext clContext = ((TACSExtension) context.tacs()).c2me$getCLContext();
        if (clContext == null) {
            return Completable.complete();
        }
        ...
        BoundedRegionArray<ProtoChunk> boundedRegionArray = new BoundedRegionArray<>(
                pos.x(),
                pos.z(),
                VkServerBatchedBiomeNoiseContext.BATCH_SIZE,
                VkServerBatchedBiomeNoiseContext.BATCH_SIZE,
        ...
        for (int dx = 0; dx < VkServerBatchedBiomeNoiseContext.BATCH_SIZE; dx++) {
            for (int dz = 0; dz < VkServerBatchedBiomeNoiseContext.BATCH_SIZE; dz++) {
        ...
        return Completable.defer(() -> {
            return Completable.fromCompletionStage(VanillaWorldGenerationDelegate.runTaskWithLockArea(
                    ...
                    () -> {
                        VkServerBatchedBiomeNoiseContext batchedBiomeNoiseContext = new VkServerBatchedBiomeNoiseContext(pos, clContext, noiseChunkGenerator, chunkGenerationContext.world().getChunkManager().getNoiseConfig());
    ...
    public KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] getDependencies(ItemHolder<ChunkPos, ChunkState, ChunkLoadingContext, ?> holder) {
        ...
        if (VkServerBatchedBiomeNoiseContext.isAligned(pos.x(), pos.z()) && !chunk.getStatus().isAtLeast(ChunkStatus.NOISE)) {
    ...
    private KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext> @NotNull [] getGenDeps(ChunkPos pos) {
        KeyStatusPair<ChunkPos, ChunkState, ChunkLoadingContext>[] deps = new KeyStatusPair[(VkServerBatchedBiomeNoiseContext.BATCH_SIZE + 16) * (VkServerBatchedBiomeNoiseContext.BATCH_SIZE + 16) - 1];
        int index = 0;
        for (int dx = -8; dx < VkServerBatchedBiomeNoiseContext.BATCH_SIZE + 8; dx++) { // padding for structure_starts
            for (int dz = -8; dz < VkServerBatchedBiomeNoiseContext.BATCH_SIZE + 8; dz++) {
                if (dx == 0 && dz == 0) continue; // skip self
                if (dx >= 0 && dx < VkServerBatchedBiomeNoiseContext.BATCH_SIZE && dz >= 0 && dz < VkServerBatchedBiomeNoiseContext.BATCH_SIZE) {
    ...
    public String toString() {
        return "c2me:batched_biome_noise_vk";
    }
