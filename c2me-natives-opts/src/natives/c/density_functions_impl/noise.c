...
static __attribute__((pure)) double c2me_natives_dfi_noise_single_op(void *instance, int x, int y, int z) {
...
c2me_natives_dfi_noise_multi_op(void *instance, double *res, noise_pos *poses, size_t length) {
    dfi_noise_data *data = instance;
    if (data->isNull) {
...
density_function_impl_data __attribute__((malloc)) *c2me_natives_create_dfi_noise_data(
