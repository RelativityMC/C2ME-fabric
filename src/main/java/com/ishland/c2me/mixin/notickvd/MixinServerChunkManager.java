...
public class MixinServerChunkManager {
    ...
    private Iterable<Entity> redirectIterateEntities(ServerWorld serverWorld) {
        final Set<ChunkPos> noTickOnlyChunks = ((IChunkTicketManager) this.ticketManager).getNoTickOnlyChunks();
        if (noTickOnlyChunks == null) return serverWorld.iterateEntities();
        return new FilteringIterable<>(serverWorld.iterateEntities(), entity -> !noTickOnlyChunks.contains(entity.getChunkPos()));
    }
