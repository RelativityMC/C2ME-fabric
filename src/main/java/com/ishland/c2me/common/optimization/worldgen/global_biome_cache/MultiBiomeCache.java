...
public class MultiBiomeCache {
    ...
    public MultiBiomeCache(Function4<Integer, Integer, Integer, MultiNoiseUtil.MultiNoiseSampler, class_6880<Biome>> delegate, IndexedIterable<Biome> registry) {
        this.registry = registry;
        this.delegate = delegate;
        this.biomeSourceCachingDelegate = new BiomeSourceCachingDelegate(this.delegate);
    }

    private final ConcurrentHashMap<MultiNoiseUtil.MultiNoiseSampler, LoadingCache<ChunkSectionPos, class_6880<Biome>[][][]>> biomeCaches = new ConcurrentHashMap<>();

    private final ThreadLocal<WeakHashMap<MultiNoiseUtil.MultiNoiseSampler, WeakHashMap<ChunkSectionPos, class_6880<Biome>[][][]>>> threadLocalCache = ThreadLocal.withInitial(WeakHashMap::new);

    private LoadingCache<ChunkSectionPos, class_6880<Biome>[][][]> createCache(MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler) {
        ...
        return CacheBuilder.newBuilder()
                ...
                .build(new CacheLoader<>() {
                    @Override
                    public class_6880<Biome>[][][] load(ChunkSectionPos key) {
                        ...
                        final class_6880<Biome>[][][] result = new class_6880[4][4][4];
    ...
    private LoadingCache<ChunkSectionPos, class_6880<Biome>[][][]> getCache(MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler) {
        return this.biomeCaches.computeIfAbsent(multiNoiseSampler, this::createCache);
    }

    private class_6880[][][] getCachedBiome(ChunkSectionPos chunkSectionPos, MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler) {
    ...
    public class_6880<Biome> getBiomeForNoiseGen(int biomeX, int biomeY, int biomeZ, MultiNoiseUtil.MultiNoiseSampler multiNoiseSampler, boolean fast) {
