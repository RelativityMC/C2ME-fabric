...
import com.ishland.c2me.mixin.optimization.reduce_allocs.chunkserialization.*;
...
public final class ChunkDataSerializer {
    ...
    private static final byte[] STRING_KROPPEB = NbtWriter.getStringBytes("Kroppeb was here :); Version: 0.2.2");
    ...
    private static void putShortListArray(ShortList[] data, NbtWriter writer, byte[] name) {
        ...
        for (ShortList shortList : data) {
            if (shortList != null) {
                ...
                writer.startFixedListEntry(0, NbtElement.NULL_TYPE);
            }
    ...
    @SuppressWarnings("unchecked")
    private static void writeBiomes(NbtWriter writer, PalettedContainer<Biome> biomeContainer, Registry<Biome> biomeRegistry) {
        writer.startCompound(STRING_BIOMES);
        // todo can this be optimized?
        // todo: does this conflict with lithium by any chance?
        var data = (PalettedContainerAccessor.PalettedContainerSerializedAccessor<Biome>) (Object)
                ((PalettedContainerAccessor<Biome>) biomeContainer).invokeWrite(
                        biomeRegistry,
                        PalettedContainer.PaletteProvider.BIOME);

        List<Biome> paletteEntries = data.getPaletteEntries();
        writer.startFixedList(STRING_PALETTE, paletteEntries.size(), NbtElement.STRING_TYPE);

        for (Biome paletteEntry : paletteEntries) {
            writer.putRegistryEntry(biomeRegistry, paletteEntry);
        }
    ...
    private static void writeStructures(
            NbtWriter writer, StructureContext context, ChunkPos pos, Map<StructureFeature<?>, StructureStart<?>> starts, Map<StructureFeature<?>, LongSet> references
    ) {
        ...
        for (Map.Entry<StructureFeature<?>, StructureStart<?>> entry : starts.entrySet()) {
            StructureFeatureAccessor feature = (StructureFeatureAccessor) entry.getKey();
            writer.startCompound(feature.getNameBytes());
        ...
        for (Map.Entry<StructureFeature<?>, LongSet> entry : references.entrySet()) {
            StructureFeatureAccessor feature = (StructureFeatureAccessor) entry.getKey();
            writer.putLongArray(feature.getNameBytes(), entry.getValue());
        }
        writer.finishCompound();
    ...
    private static void writeStructureStart(NbtWriter writer, StructureStartAccessor structureStart, StructureContext context, ChunkPos pos) {
        ...
        writer.putRegistry(STRING_ID, Registry.STRUCTURE_FEATURE, structureStart.getFeature());
